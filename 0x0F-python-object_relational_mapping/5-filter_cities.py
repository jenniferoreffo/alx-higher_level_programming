...
"""                                                         Access to the database and get the cities               from the database.                                  """
    ...
    with db.cursor() as cur:
        cur.execute("""SELECT cities.id, cities.name, states.name FROM cities JOIN states ON cities.states_id  WHERE state_name LIKE BINARY %(state_name) ORDER BY cities.id ASC """, {'state_name': argv[4]})
      rows = cur.fetchall()
if rows is not None:
